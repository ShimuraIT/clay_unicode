...
{
	Vector2 windowsz={.x=GetScreenWidth(),.y=GetScreenHeight()};
    for (int j = 0; j < renderCommands.length; j++)
    {
        ...
        switch (renderCommand->commandType)
        {
            ...
            case CLAY_RENDER_COMMAND_TYPE_IMAGE: {
				//offscreen textures already don't render!
				/*if(!(
					(boundingBox.x+boundingBox.width<0)||
					(boundingBox.y+boundingBox.height<0)||
					(boundingBox.x>windowsz.x)||
					(boundingBox.y>windowsz.y)
				)){*/
            ...
            case CLAY_RENDER_COMMAND_TYPE_RECTANGLE: {
				if(!(
					...
					//oh, this was pointless! please... need funny rounded corners like 2013 mojang logo... makes me laugh... tee hee...
					float cornersum=config->cornerRadius.topLeft+config->cornerRadius.topRight+config->cornerRadius.bottomLeft+config->cornerRadius.bottomRight;
					if(cornersum > 0) {
					...
					}else if(config->isUnicodeText){
						Clay_Unicode stringInfo=config->stringInfo;
						Font fontToUse = fonts[stringInfo.fontId];
						Font highlightFont = fonts[stringInfo.highlightFontId];
						DrawTextBoxedSelectable(
							fontToUse,
							highlightFont,
							stringInfo.stringContents,
							(Rectangle){boundingBox.x,boundingBox.y,boundingBox.width,boundingBox.height},
							stringInfo.fontSize,
							stringInfo.lineHeight,
							stringInfo.letterSpacing,
							stringInfo.wordWrap,
							CLAY_COLOR_TO_RAYLIB_COLOR(stringInfo.textColor),
							stringInfo.highlightStart,
							stringInfo.highlightLength,
							CLAY_COLOR_TO_RAYLIB_COLOR(stringInfo.highlightColor),
