...
typedef struct Clay_Unicode {
	// A string composed of codepoints ;)
	char* stringContents;
	...
	// stuff for highlights
	uint16_t highlightFontId;
	uint16_t highlightStart;
	uint16_t highlightLength;
	Clay_Color highlightColor;
	//Clay_Color highlightBackgroundColor;// Use the rectangle's backgroundColor variable

	// Alignment
	Clay_LayoutAlignmentX halign;
	Clay_LayoutAlignmentY valign;
...
typedef struct Clay_RectangleRenderData {
	...
	// unicode piggybacks off of rectangles
	bool isUnicodeText;
	Clay_Unicode stringInfo;
...
/**
	* Sets up a ClayUnicode variable in a straightforward function.
	* 
	* @param stringContents i.e the string to show. const works.
	* @param textColor
	* @param fontId
	* @param fontSize
	* @param lineHeight usually the same as fontSize
	* @param letterSpacing set to something like f0.0 or f1.0 (TODO: explain more)
	* @param wordWrap
	* @param halign
	* @param valign
	* @param hasHighlights set to 1 if you want to highlight some text, then: [uint16_t highlightFontId, Clay_Color highlightStart, Clay_Color highlightLength, Clay_Color highlightColor]
	
*/
Clay_Unicode Clay_MakeUnicode(char* stringContents,Clay_Color textColor,uint16_t fontId,float fontSize,float lineHeight,float letterSpacing, bool wordWrap, Clay_LayoutAlignmentX halign, Clay_LayoutAlignmentY valign, uint8_t hasHighlights, ...);


Clay_Unicode Clay_MakeUnicode(char* stringContents,Clay_Color textColor,uint16_t fontId,float fontSize,float lineHeight,float letterSpacing, bool wordWrap, Clay_LayoutAlignmentX halign, Clay_LayoutAlignmentY valign, uint8_t hasHighlights, ...){
	Clay_Unicode r;
	r.stringContents=stringContents;
	...
	if(hasHighlights){
		va_list highlightinfo;
		hasHighlights=4;
		va_start(highlightinfo,hasHighlights);//goofy because i got a warning
		r.highlightFontId=va_arg(highlightinfo,int);
		r.highlightStart=va_arg(highlightinfo,int);
		r.highlightLength=va_arg(highlightinfo,int);
		r.highlightColor=va_arg(highlightinfo,Clay_Color);
		va_end(highlightinfo);
	}else{
		r.highlightFontId=fontId;
		r.highlightColor=textColor;
		r.highlightStart=0;
		r.highlightLength=0;
	}
	return r;
	...
	for (int32_t rootIndex = 0; rootIndex < context->layoutElementTreeRoots.length; ++rootIndex) {
		...
		while (dfsBuffer.length > 0) {
			...
			if (!context->treeNodeVisited.internalArray[dfsBuffer.length - 1]) {
				...
				if (emitRectangle) {
					Clay_RectangleRenderData temprect;
					temprect.backgroundColor = sharedConfig->backgroundColor;
					temprect.cornerRadius = sharedConfig->cornerRadius;
					temprect.isUnicodeText = false;
					if(sharedConfig->userData!=NULL){
						temprect.isUnicodeText = true;
						Clay_Unicode* sinfo=sharedConfig->userData;
						//sinfo->highlightBackgroundColor=sharedConfig->backgroundColor;
						temprect.stringInfo=*sinfo;
					}
...
Here's me breaking rule 3 for rule 2 to say that this is altered to support
unicode in a very weird way that I would not recommend.

Reasons not to use this:
- It piggybacks off of Clay rectangles (which don't technically exist anymore)
- It doesn't clean up with the rest of Clay's mallocs.
- It is not renderer agnostic. Well, maybe it is, but I implemented the display
functionality in raylib.

To use it anyways, do this:

	ClayUnicode texthere=Clay_MakeUnicode(...);
